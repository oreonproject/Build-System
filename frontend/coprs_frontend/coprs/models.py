...
# ISO Builder Models for Oreon Build System

class ISOProject(db.Model, helpers.Serializer):
    """
    Represents an ISO project for grouping related ISO builds.
    """
    __tablename__ = "iso_project"
    ...
    user = db.relationship("User", backref=db.backref("iso_projects"))
    
    # Timestamps
    created_on = db.Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
    
    # Project settings
    default_architecture = db.Column(db.String(20), default="x86_64")
    default_release_version = db.Column(db.String(20), default="10")
    
    __table_args__ = (
        db.Index('iso_project_user_name', 'user_id', 'name', unique=True),
    )

    @property
    def full_name(self):
        """Return full project name in user/project format."""
        return f"{self.user.name}/{self.name}"

    @property
    def builds_count(self):
        """Return the number of builds in this project."""
        return len(self.iso_builds)

    @property
    def latest_build(self):
        """Return the most recent build."""
        if self.iso_builds:
            return sorted(self.iso_builds, key=lambda x: x.submitted_on, reverse=True)[0]
        return None
    ...
    def to_dict(self):
        return {
            ...
            'default_release_version': self.default_release_version,
        }


class ISOBuild(db.Model, helpers.Serializer):
    """
    Represents a single ISO build using livemedia-creator.
    """
    __tablename__ = "iso_build"
    ...
    architecture = db.Column(db.String(20), nullable=False, default="x86_64")
    volid = db.Column(db.String(100), nullable=False)
    release_version = db.Column(db.String(20), nullable=False, default="10")
    
    # Kickstart source (either file path or inline content)
    kickstart_path = db.Column(db.Text)  # Path to uploaded .ks file
    kickstart_content = db.Column(db.Text)  # Inline kickstart content
    
    # Build options stored as JSON
    build_options = db.Column(db.Text)  # JSON string with build options
    
    # Build status and timestamps
    status = db.Column(db.String(20), nullable=False, default="pending", index=True)
    progress = db.Column(db.Integer, default=0)  # Progress percentage (0-100)
    
    submitted_on = db.Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow, index=True)
    started_on = db.Column(db.DateTime, index=True)
    ended_on = db.Column(db.DateTime, index=True)
    
    # Build results
    result_dir = db.Column(db.Text)  # Directory containing build results
    result_url = db.Column(db.Text)  # URL to download the ISO
    iso_size = db.Column(db.BigInteger)  # Size of the resulting ISO in bytes
    
    # Build logs and output
    build_log = db.Column(db.Text)  # Build log content
    error_message = db.Column(db.Text)  # Error message if build failed
    
    # Relations
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False, index=True)
    user = db.relationship("User", backref=db.backref("iso_builds"))
    
    project_id = db.Column(db.Integer, db.ForeignKey("iso_project.id"), nullable=True, index=True)
    project = db.relationship("ISOProject", backref=db.backref("iso_builds"))
    
    # Task management
    task_id = db.Column(db.String(100), index=True)  # Background task ID
    
    __table_args__ = (
        db.Index('iso_build_status_submitted_on', 'status', 'submitted_on'),
        db.Index('iso_build_user_submitted_on', 'user_id', 'submitted_on'),
    )

    @property
    def build_options_dict(self):
        """Return build options as a dictionary."""
        if self.build_options:
            try:
                return json.loads(self.build_options)
    ...
    def duration_humanized(self):
        """Return human-readable build duration."""
    ...
    def is_finished(self):
        """Return True if the build has finished (successfully or not)."""
        return self.status in ['succeeded', 'failed', 'canceled']

    @property
    def is_running(self):
        """Return True if the build is currently running."""
    ...
    @property
    def kickstart_source(self):
        """Return the kickstart source (file or content)."""
        if self.kickstart_path:
            return f"File: {os.path.basename(self.kickstart_path)}"
        elif self.kickstart_content:
            return "Inline content"
        return "No kickstart provided"

    @property
    def iso_filename(self):
        """Generate the expected ISO filename."""
        return f"{self.name}-{self.version}-{self.release_version}.{self.architecture}.iso"

    @property
    def full_name(self):
        """Return full build name including user and project."""
        if self.project:
            return f"{self.project.full_name}/{self.name}-{self.version}"
        return f"{self.user.name}/{self.name}-{self.version}"

    def to_dict(self, include_logs=False):
        """Convert build to dictionary representation."""
        data = {
            'id': self.id,
            'name': self.name,
            'full_name': self.full_name,
            ...
            'duration': self.duration,
            'duration_humanized': self.duration_humanized,
            'kickstart_source': self.kickstart_source,
...
class ISOTemplate(db.Model, helpers.Serializer):
    """
    Represents a kickstart template for ISO building.
    """
    __tablename__ = "iso_template"

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False, unique=True, index=True)
    description = db.Column(db.Text)
    category = db.Column(db.String(50), default="general")  # e.g., "desktop", "server", "minimal"
    
    # Template content
    kickstart_content = db.Column(db.Text, nullable=False)
    
    # Template metadata
    supported_architectures = db.Column(db.Text, default="x86_64")  # Comma-separated list
    default_release_version = db.Column(db.String(20), default="10")
    
    # Template settings
    is_public = db.Column(db.Boolean, default=True, nullable=False)
    is_official = db.Column(db.Boolean, default=False, nullable=False)  # Official Oreon templates
    
    # Relations
    created_by_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False, index=True)
    created_by = db.relationship("User", backref=db.backref("iso_templates"))
    ...
    __table_args__ = (
        db.Index('iso_template_category_name', 'category', 'name'),
        db.Index('iso_template_public_official', 'is_public', 'is_official'),
    )

    @property
    def supported_architectures_list(self):
        """Return supported architectures as a list."""
        if self.supported_architectures:
            return [arch.strip() for arch in self.supported_architectures.split(',')]
        return ['x86_64']

    @property
    def usage_count(self):
        """Return the number of times this template has been used."""
        # This would require tracking template usage in builds
        return 0  # Placeholder for now

    def supports_architecture(self, arch):
        """Check if template supports the given architecture."""
        return arch in self.supported_architectures_list
    ...
    def to_dict(self, include_content=False):
        ...
        data = {
            ...
            'supported_architectures': self.supported_architectures_list,
            'default_release_version': self.default_release_version,
        ...
        if include_content:
            data['kickstart_content'] = self.kickstart_content
