...
# ISO Builder Forms for Oreon Build System

class ISOProjectForm(BaseForm):
    """
    Form for creating and editing ISO projects
    """
    name = wtforms.StringField(
        "Project Name",
        validators=[
    ...
    description = wtforms.TextAreaField(
        "Description",
        validators=[wtforms.validators.Optional()],
        description="Brief description of what this ISO project is for")
    
    default_architecture = wtforms.SelectField(
    ...
    default_release_version = wtforms.StringField(
        ...
        description="Default Oreon release version (e.g., 10, 11)")


class ISOBuildForm(BaseForm):
    """
    Form for creating ISO builds
    """
    iso_name = wtforms.StringField(
        "ISO Name",
        validators=[
            wtforms.validators.DataRequired(),
            NameCharactersValidator(),
        ...
        description="Name for your ISO image")
    ...
    version = wtforms.StringField(
        ...
        description="Version identifier (e.g., 1.0, beta1)")
    
    architecture = wtforms.SelectField(
        "Architecture",
        choices=[
            ("x86_64", "x86_64"),
            ("aarch64", "aarch64"),
    ...
    volid = wtforms.StringField(
        "Volume ID",
        validators=[
            wtforms.validators.DataRequired(),
            wtforms.validators.Length(min=1, max=100)
        ],
        description="Volume identifier for the ISO (appears in file managers)")
    
    release_version = wtforms.StringField(
        "Release Version",
        default="10",
        validators=[
            wtforms.validators.DataRequired(),
        ...
        description="Oreon release version to base ISO on")
    
    # Kickstart source options
    kickstart_file = FileField(
        "Kickstart File",
        validators=[wtforms.validators.Optional()],
        description="Upload a .ks kickstart file")
    
    kickstart_content = wtforms.TextAreaField(
        "Kickstart Content",
        validators=[wtforms.validators.Optional()],
        description="Or paste kickstart content directly",
        render_kw={'rows': 15, 'placeholder': 'Paste your kickstart file content here...'})
    
    # Build options
    make_iso = wtforms.BooleanField(
        "Create ISO image",
        default=True,
        false_values=FALSE_VALUES,
        description="Create bootable ISO image")
    
    iso_only = wtforms.BooleanField(
        "ISO only",
        default=False,
        false_values=FALSE_VALUES,
        description="Only create ISO, skip disk image creation")
    
    no_virt = wtforms.BooleanField(
        "Disable virtualization",
        default=False,
        false_values=FALSE_VALUES,
        description="Disable virtualization for the build (slower but more compatible)")
    
    nomacboot = wtforms.BooleanField(
        "Disable Mac boot support",
        default=False,
        false_values=FALSE_VALUES,
        description="Don't include Mac boot support in the ISO")
    ...
    timeout = wtforms.IntegerField(
        ...
        validators=[
            wtforms.validators.Optional(),
            wtforms.validators.NumberRange(min=300, max=14400)  # 5 minutes to 4 hours
        ],
        default=3600,
        description="Maximum time allowed for the build (default: 1 hour)")
    
    memory = wtforms.IntegerField(
        "Memory (MB)",
        validators=[
            wtforms.validators.Optional(),
            wtforms.validators.NumberRange(min=1024, max=8192)  # 1GB to 8GB
        ],
        default=2048,
        description="Memory allocation for the build VM (default: 2GB)")
    
    def validate(self, extra_validators=None):
        """Custom validation to ensure either file or content is provided"""
        valid = super().validate(extra_validators)
        
        # Check that either kickstart file or content is provided
        if not self.kickstart_file.data and not self.kickstart_content.data:
            self.kickstart_file.errors.append("Either upload a kickstart file or provide kickstart content")
            self.kickstart_content.errors.append("Either upload a kickstart file or provide kickstart content")
            valid = False
        
        # If both are provided, warn user
        if self.kickstart_file.data and self.kickstart_content.data:
            self.kickstart_content.errors.append("Please provide either a file OR content, not both")
            valid = False
        
        # Validate kickstart file extension if uploaded
        if self.kickstart_file.data:
            filename = self.kickstart_file.data.filename.lower()
            if not filename.endswith('.ks'):
                self.kickstart_file.errors.append("Kickstart file must have .ks extension")
                valid = False
